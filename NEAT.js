...
function NEAT(config) {
	...
	this.getDecisions = function () { // Returns every creature's decision index in an array.
		let result = [];
...
function Creature(model) {
	...
	this.getWeightsForLayer = function (layer) {
		layer -= 1

		output = []

		for (let w = 0; w < this.network.layers[layer].nodes.length; w++) {
			tempOutput = []
			for (let e = 0; e < this.network.layers[layer].nodes[w].weights.length; e++) {
				tempOutput.push(activation.SIGMOID(this.network.layers[layer].nodes[w].weights[e]))
			}
	...
	this.getOutputLayerValues = function () {
		return this.network.layers[this.network.layers.length - 1].nodes.map(_ => _.value)
	}

	this.getNodeValuesForLayer = function (layer) {
		layer -= 1
		return this.network.layers[layer].nodes.map(_ => _.value)
	...
	this.decision = function () { // Some spaghetti code that returns the decision of the creature.
		let index = -1;
		let max = -Infinity;
